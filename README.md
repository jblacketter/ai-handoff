...
- **Cycle region** (3.7) — a running handoff is unmistakable: **Lead** and **Reviewer** lanes with the turn token on the side we are waiting on and the working agent's lane pulsing, then the **Activity** log of every turn (both roles' turns and reviews, pre-checks, review lenses, decision briefs, chats) — the running one streams its log, the finished ones *stay* with a plain outcome (`done · cancelled · failed · timed out · process disappeared · no result recorded`) and a log link. **Start** acknowledges within one refresh; the chat keeps a finished turn's activity under a disclosure instead of dropping it.

<p align="center"><img src="docs/media/screenshots/cockpit-cycle.png" alt="Cockpit during a running handoff: the project-first strip (demo-web · the phase · Codex is working · watcher on), the quiet Needs-you line, the Cycle region with the Claude lane waiting and the Codex lane working (pulsing), and the Activity log — the review streaming its log, a passed pre-check and finished turns below it" width="100%"></p>

The page reads top to bottom in attention order and in your words: the **Now** strip (the project first — you can serve the wrong one — then the phase and round, who is working or who we are waiting on, the watcher on / off with its one action, the connection); the **Needs you** banner — one card per thing only the human can do, red only when a human is required (an escalation, a question, paused turns, a lost process, a turn waiting while the watcher is off), a quiet one-liner otherwise; the **Cycle** region; the tabs — *\<your lead\>* (the chat), Rounds, Diff, Usage, Notes. Every button is the CLI command with the same effect, recorded as `by = web:<user>`. The cockpit binds **127.0.0.1** by default and guards every POST with a per-run page token; `--host 0.0.0.0` exposes it on the network — including the ability to run agent turns and launch processes — and is your call. Two `tagteam serve` on one port never shadow each other: the second refuses and names the first. Zones, the launchpad, the lead conversation model and the security model in full: [the cockpit](docs/how-tagteam-works.md#cockpit), [talking to the lead](docs/how-tagteam-works.md#lead).
