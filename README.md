...
- **Cycle region** (3.7) — a running handoff is unmistakable: **Lead** and **Reviewer** lanes with the turn token on the owed side and the running agent's lane pulsing, then the **Activity** log of every agent turn (both roles' cycle turns, gates, panel lenses, briefs, lead conversations) — the running one streams its log, the finished ones *stay* with a named outcome (`finished · cancelled · failed · timed out · process gone · orphaned`) and a log link. **Start** acknowledges within one refresh; the Lead panel keeps a finished turn's activity under a disclosure instead of dropping it.

<p align="center"><img src="docs/media/screenshots/cockpit-cycle.png" alt="Cockpit during a running handoff: the Cycle region with the Lead lane waiting and the Reviewer lane on turn and running (pulsing), the status line, and the Activity log — the reviewer's cycle turn streaming its log, a passed gate and three finished turns below it" width="100%"></p>

The **Now** strip (phase / type / round, whose turn and for how long, the in-flight turn and its kind, the last turn's outcome, the pause hold, watcher liveness with **Start / Stop**, connection mode); **Needs you** — one card per thing only the human can do, each with the buttons and the exact CLI they run; **Watch** tabs — Lead, Feed, Diff, Usage, Notes. Every button is the CLI command with the same effect, recorded as `by = web:<user>`. The cockpit binds **127.0.0.1** by default and guards every POST with a per-run page token; `--host 0.0.0.0` exposes it on the network — including the ability to run agent turns and launch processes — and is your call. Two `tagteam serve` on one port never shadow each other: the second refuses and names the first. Zones, the launchpad, the lead conversation model and the security model in full: [the cockpit](docs/how-tagteam-works.md#cockpit), [talking to the lead](docs/how-tagteam-works.md#lead).
