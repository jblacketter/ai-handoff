...
**Verification budget (impl cycles).** Run focused tests while you work; run the project's full suite **once**, right before you submit, and state the result in your submission (`full suite: N passed, M skipped @ <commit>`). If the gatekeeper is enabled, `tagteam gate check` *is* that one full run — do not run the suite and then the gate check on the same tree. Keep the submission to what the reviewer needs (what changed, where, how it was verified); the reviewer reads the diff, not a narrative.
...
1. Read the lead's submission: `tagteam cycle rounds --phase [phase] --type [plan|impl] --tail 1` (use `--tail 2`, or no `--tail`, only when earlier rounds matter for this review)
   ...
   - **Do not re-run the full suite.** When the submission (or a gate entry) reports a full-suite result for the submitted commit, take it as fact — the lead is accountable for it, and the suite is expensive. Verify by reading the diff and, at most, running the test files it touches. Re-run the whole suite only if you have a concrete reason to doubt the report (the reported commit differs from HEAD, the numbers do not add up, a focused run fails) — and say why in your entry.
