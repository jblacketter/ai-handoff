...
- [ ] **(gates the tag, not approval — revised impl r2)** `.github/workflows/tests.yml` runs the full suite on ubuntu-latest and windows-latest and is green, including all headless fake-agent tests on Windows through the `.cmd` shim path. Workflow committed; first run happens on Jack's push. (Reviewer decision: this CI is the gating Windows criterion; a real signed-in CLI smoke on Windows is best-effort and the findings doc + release notes state explicitly whether it happened.)
