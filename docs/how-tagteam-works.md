...
- **Now** strip — phase / type / round, whose turn and for how long, the agent process **in flight and what kind of turn it is** (`cycle turn r2` · `lead conversation` · `gate` · `panel lens` · `brief`), the **last** turn and how it ended, the pause hold, watcher liveness, queued notes, and the connection mode (**Live** via SSE / **Polling** fallback / Disconnected).
- **Needs you** — one card per thing only the human can do: an escalation with its brief and **Approve / Request changes**, a needs-human question with **Answer**, a hold with **Resume**, a missing/failed brief with **Generate brief**, a stale in-flight or missing watcher with the CLI to run. Empty when nothing needs you — and it says so. Clicking **Start** answers within one refresh: the card is replaced by the Cycle region's *starting* state (below) while the lead runs the first command; if that turn fails, the card is back with the failure under it.
- **Cycle** region (3.7, top of Watch) — the running handoff as one thing: two lanes, **Lead** and **Reviewer**, with the turn token on the owed side and the lane whose agent process is running pulsing (`on turn · running cycle turn r2 · 1m12s`), a status line (`phase · type · round N · owed to <role> for <age> · last: <role> <kind> <outcome> <age> ago`), and the **Activity** log — every agent turn recorded for the project, newest first: both roles' cycle turns, gate runs, panel lenses, briefer runs and lead-conversation turns. The running row streams its log live (`[log]`, `[cancel]`); a finished row **stays**, with a named outcome from one vocabulary used everywhere in the page — `running · finished · cancelled · failed · timed out · process gone · orphaned` — its duration and `[log]` / `[transcript]`. Rows are keyed and patched in place, never rebuilt, so the running turn's lines survive its end and the page's refreshes (until a reload; the log file is the record). This replaces the tail drawer; `tagteam tail` is unchanged. Read model: `GET /api/activity` (read-only over the in-flight marker, `usage`, `conversation_turns`, `gates`, `panels`, `launches`; no schema change) and `GET /api/activity/log/<stem>/events` (SSE over `.tagteam/turns/<stem>.log`, byte-offset ids, `Last-Event-ID` replay, `end` once the turn's marker is gone and the file drained).
- **Watch** tabs — **Lead** (below), **Feed** (live round stream: entries, rulings, interjections, briefs), **Diff** (scope-diff of the current submission, per file, capped), **Usage** (round-over-round churn per role — no threshold line, since auto-escalation is a count of consecutive stale rounds and not a round number — burn by role / cycle / process, and the Claude subscription-window signal), **Notes** (interjections: queue one, retire one).
